...
whoami	-посмотреть имя текущего пользователя
...
sudo passwd user_1 (или passwd user_1 для суперпользователя)
...
su - user_2
	-войти под пользователем user_2
	
sudo userdel user_1
	-удаление пользователя user_1

remove-home
	-Для удаления домашнего каталога может использоваться дополнительный параметр

remove-all-files
	-для поиска и удаления всех файлов системы, принадлежащих удаляемому пользователю используется параметр
...
usermod
	...
	usermod -aG group_John John  -добавляет пользователя John
...
sudo chown :group user_1 script1			-смена владельца группы файла на user_1
